...
class TransactionUtil extends Util
{
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'tax_id' => isset($product['tax_id']) ? $product['tax_id'] : null,
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        if (!empty($payment['method']) && $payment['method'] === 'cheque') {
            $contact_id = !empty($transaction->contact_id)
                ? $transaction->contact_id
                : TransactionPayment::where('id', $payment_id)->value('payment_for');

            if (!empty($contact_id) && Contact::where('id', $contact_id)->value('is_default') == 1) {
                throw new ChequePaymentNotAllowedForWalkInCustomer(__('lang_v1.cheque_payment_requires_registered_customer'));
            }
        }
        ...
        if (!empty($payment['cheque_issue_date'])) {
            $payment['cheque_issue_date'] = $uf_data ? $this->uf_date($payment['cheque_issue_date'], true) : $payment['cheque_issue_date'];
        }

        if (!empty($payment['cheque_passing_date'])) {
            $payment['cheque_passing_date'] = $uf_data ? $this->uf_date($payment['cheque_passing_date'], true) : $payment['cheque_passing_date'];
        }

        if (!empty($payment['method']) && $payment['method'] === 'cheque' && empty($payment['cheque_status'])) {
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            //Note: Receipts already render `all_due` in most layouts.
            //We set it for non-default customers to show their cumulative due on the printable bill.
            if (!empty($transaction->contact_id)) {
                $business_id_for_due = is_object($business_details) ? $business_details->id : ($business_details['id'] ?? null);
                $contact = Contact::select('id', 'type', 'is_default', 'business_id')
