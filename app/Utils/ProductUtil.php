        ...
        $query = Discount::where('business_id', $business_id)
            ...
            ->where(function ($q) use ($product, $variation_id) {
                $q->where(function ($sub_q) use ($product) {
                    $sub_q->where(function ($q_brand) use ($product) {
                        if (!empty($product->brand_id)) {
                            $q_brand->where('brand_id', $product->brand_id)
                                ->orWhereNull('brand_id');
                        } else {
                            $q_brand->whereNull('brand_id');
                        }
                    })->where(function ($q_category) use ($product) {
                        if (!empty($product->category_id)) {
                            $q_category->where('category_id', $product->category_id)
                                ->orWhereNull('category_id');
                        } else {
                            $q_category->whereNull('category_id');
                        }
