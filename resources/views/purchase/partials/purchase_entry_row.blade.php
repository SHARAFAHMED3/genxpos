    ...
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        @else
            @if(empty($is_purchase_order))
                ...
                <td>
                    <span class="row_selling_line_total display_currency">0</span>
                </td>
            @endif
        @endif
            ...
            <td style="text-align: left;">
                ...
                @if(!empty($expiry_period_type))
                    ...
                    @if(session('business.expiry_type') == 'add_manufacturing')
                        @php $hide_mfg = false; @endphp
                    @else
                        @php $hide_mfg = true; @endphp
                    @endif
                ...
                @else
                    <div class="text-center">@lang('product.not_applicable')</div>
                @endif
            </td>
        @endif

        <td><i class="fa fa-trash remove_purchase_entry_row text-danger" title="@lang('messages.remove')" style="cursor:pointer"></i></td>
    </tr>
